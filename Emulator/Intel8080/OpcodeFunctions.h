...
void MOV(State8080* state, uint8_t opcode)
{
   ...
   state->setRegister(dst, state->getRegister(src));
}
...
void PUSH(State8080* state, uint8_t first, uint8_t second)
{
   ...
   state->memory->write(--state->Reg.sp, first);

   // Contents of second register are saved at (sp-2)
   state->memory->write(--state->Reg.sp, second);
...
void POP(State8080* state, uint8_t &first, uint8_t &second)
{
   ...
   second = state->memory->read(state->Reg.sp++);

   // Contents of first register are restored from (sp+1)
   first = state->memory->read(state->Reg.sp++);
...
void RET(State8080* state)
{
   ...
   uint16_t addr = 0;
   addr |= state->memory->read(state->Reg.sp++);
   addr |= state->memory->read(state->Reg.sp++) << 8;
