...
{
   unsigned char *code = &memory[Reg.pc];
...
void State8080::displayAbrev()
{
   ...
   std::cout
      ...
      << " A=" << std::setw(2) << A
      << " BC=" << std::setw(4) << BC
      << " (DE=" << std::setw(4) << DE << ")=" << std::setw(2) << (int)memory[DE]
      << " (HL=" << std::setw(4) << HL << ")=" << std::setw(2) << (int)memory[HL]
      << " (SP=" << std::setw(4) << SP << ")=";
   if (0x2300 <= SP && SP < 0x2400) // Stack pointer must point to stack area
      std::cout << std::setw(4) << (int)(memory[SP + 1] << 8 | memory[SP]);
   std::cout << std::dec;
}
